...
import { useMapStore } from '#imports'
...
onMounted(async () => {
  ...
  townName.value = mapStore.state.townName;
  postalCode.value = mapStore.state.postalCode;

  const erpsGenerator = accessLibreStore.getERPSbyPostalCode(postalCode.value);
...
<template>
  ...
  <div class="sm:w-4/5 mx-auto pl-2">

    <h1 class="text-lg sm:text-3xl">Établissements à <span class="font-bold">{{ townName }}</span> ({{ postalCode }})</h1>

    <p class="italic text-sm">Il y a {{ erpsCount }} établissements</p>

    <nav class="flex gap-3 flex-wrap mt-4">

    </nav>

    <div v-if="categories === null || categories.length === 0" class="flex justify-center items-center h-3/5">
      <PrimeProgressSpinner />
    </div>

    <PrimePanel v-else  header="Catégories" toggleable>
      <div class="flex gap-3 flex-wrap mt-4">
        <div v-for="category in categories" class="relative">
          <PrimeChip class="bg-[#C2E0FAC9] border-2 border-[#2093F750] text-center text-xs" :label="category.name" />
          <PrimeBadge class="absolute right-[-14px] top-[-10px]" :value="category.count" severity="contrast"></PrimeBadge>
        </div>
      </div>
    </PrimePanel>
  ...
  <div class="sm:w-4/5 mx-auto h-screen">
    ...
    <PrimeDataView v-else :value="erps" paginator :pageLinkSize="1" :rows="10" dataKey="id" class="my-8">
          ...
          <div v-for="(item, index) in slotProps.items" :key="index" class="col-12">
            ...
            <div class="my-4 mx-2 sm:mx-10 border-b-2 sm:flex justify-between">
              ...
              <div>
                ...
                <PrimeChip class="bg-[#C2E0FAC9] border-2 border-[#2093F750] text-center text-xs" :label="item.activite.nom" />
              ...
              <div class="flex justify-center items-center my-4 sm:w-3/5">
                ...
                <a href="#searchMap" class="mr-2">
                  <PrimeButton icon="pi pi-map-marker" label="Voir carte" severity="secondary"></PrimeButton>
                </a>

                <a v-if="item.site_internet !== '' && item.site_internet !== null" :href="item.site_internet" target="_blank">
                  <PrimeButton icon="pi pi-external-link" label="Site Web"></PrimeButton>
                </a>

                <a v-else href="#">
                  <PrimeButton icon="pi pi-question" label="Ajouter" severity="secondary"></PrimeButton>
                </a>
